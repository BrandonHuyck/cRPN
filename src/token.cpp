...
std::string to_string(const Token &token)
{
    std::unordered_map<TokenType, std::string> ts = {
        ...
        // values
        {TokenType::INT_VAL, "INT_VAL"},
        {TokenType::DOUBLE_VAL, "DOUBLE_VAL"},
        {TokenType::CHAR_VAL, "CHAR_VAL"},
        {TokenType::STRING_VAL, "STRING_VAL"},
        {TokenType::BOOL_VAL, "BOOL_VAL"},
        {TokenType::NULL_VAL, "NULL_VAL"},
        // primitive types
        {TokenType::INT_TYPE, "INT_TYPE"},
        {TokenType::DOUBLE_TYPE, "DOUBLE_TYPE"},
    ...
    return std::to_string(token.line()) + ", " + std::to_string(token.column()) + ": " + ts[token.type()] + " '" + token.lexeme() + "'";
}
